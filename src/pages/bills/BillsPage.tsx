...
import { FileText, Plus, Search, ChevronRight, Check, X, Pause, Send } from 'lucide-react';
...
const BillsPage: React.FC = () => {
  ...
  const selected = bills.find(b => b.id === selectedId || b.bill_number === selectedId) || null;
  ...
  const handleAction = (status: BillStatus) => {
    if (selected) {
      DataStore.updateBillStatus(selected.id, status);
    }
  };
        ...
        <div className="table-container">
          <div className="table-toolbar">
            ...
            <select className="filter-select" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}>
              ...
              {['draft','submitted','voting','passed','rejected','suspended','awaiting_president','approved','enacted','archived'].map(s => (
                ...
                {filtered.map(bill => (
                  ...
                  >
                    ...
                    <td style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>
                      {bill.created_at ? format(new Date(bill.created_at), 'MMM dd') : 'Recent'}
                    </td>
            ...
            {role !== 'public' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                {selected.status === 'draft' && (
                  <button className="btn btn-primary btn-sm" onClick={() => handleAction('submitted')}>
                    <Send size={13} /> Submit to Parliament
                  </button>
                )}
                {selected.status === 'submitted' && (
                  <button className="btn btn-primary btn-sm" onClick={() => handleAction('voting')}>
                    <Send size={13} /> Open Parliament Voting
                  </button>
                )}
                {selected.status === 'voting' && (
                  <button className="btn btn-warning btn-sm" onClick={() => handleAction('awaiting_president')}>
                ...
                {(selected.status === 'awaiting_president' || selected.status === 'voting' || selected.status === 'passed') && (
                  <>
                    ...
                    <button className="btn btn-warning btn-sm" onClick={() => handleAction('suspended')}>
                      <Pause size={13} /> Suspend Bill
                    </button>
                  </>
                )}
                {(selected.status === 'suspended' || selected.status === 'rejected') && (
                  <button className="btn btn-secondary btn-sm" onClick={() => handleAction('voting')}>
                    Re-open Voting
                  </button>
                )}
              </div>
