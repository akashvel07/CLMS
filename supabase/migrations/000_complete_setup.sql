-- ═══════════════════════════════════════════════════════════════
-- CLMS COMPLETE SETUP — Paste this ENTIRE file into Supabase SQL Editor
-- Run in ONE shot. Project: mxltfkrcxxhqpblrbkyz
-- ═══════════════════════════════════════════════════════════════
...
-- USERS policies
CREATE POLICY "president_full_users"       ON users FOR ALL    USING (current_user_role() = 'president');
CREATE POLICY "own_profile_select"         ON users FOR SELECT USING (id = auth.uid());
CREATE POLICY "own_profile_update"         ON users FOR UPDATE USING (id = auth.uid());

-- MINISTRIES policies
CREATE POLICY "all_read_ministries"        ON ministries FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "president_manage_min"       ON ministries FOR ALL    USING (current_user_role() = 'president');

-- BILLS policies
CREATE POLICY "all_read_bills"             ON bills FOR SELECT USING (auth.uid() IS NOT NULL AND status != 'deleted');
CREATE POLICY "ministry_create_bill"       ON bills FOR INSERT WITH CHECK (current_user_role() = 'ministry' AND ministry_id = current_user_ministry() AND created_by = auth.uid());
CREATE POLICY "ministry_update_draft"      ON bills FOR UPDATE USING (current_user_role() = 'ministry' AND ministry_id = current_user_ministry() AND status = 'draft');
CREATE POLICY "president_manage_bills"     ON bills FOR ALL    USING (current_user_role() = 'president');
CREATE POLICY "public_read_enacted"        ON bills FOR SELECT USING (status = 'enacted');

-- LAWS policies
CREATE POLICY "all_read_laws"              ON laws FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "president_manage_laws"      ON laws FOR ALL    USING (current_user_role() = 'president');
CREATE POLICY "public_read_laws"           ON laws FOR SELECT USING (true);

-- PARLIAMENT VOTES policies
CREATE POLICY "all_read_votes"             ON parliament_votes FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "cast_vote"                  ON parliament_votes FOR INSERT WITH CHECK (auth.uid() IS NOT NULL AND user_id = auth.uid());

-- MINISTRY REVIEWS policies
CREATE POLICY "all_read_reviews"           ON ministry_reviews FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "ministry_submit_review"     ON ministry_reviews FOR INSERT WITH CHECK (current_user_role() = 'ministry' AND ministry_id = current_user_ministry());
CREATE POLICY "president_manage_reviews"   ON ministry_reviews FOR ALL    USING (current_user_role() = 'president');

-- REQUESTS policies
CREATE POLICY "ministry_read_requests"     ON requests FOR SELECT USING (current_user_role() IN ('ministry','president'));
CREATE POLICY "ministry_create_request"    ON requests FOR INSERT WITH CHECK (current_user_role() = 'ministry' AND from_ministry = current_user_ministry());
CREATE POLICY "ministry_update_request"    ON requests FOR UPDATE USING (current_user_role() = 'ministry' AND to_ministry = current_user_ministry());
CREATE POLICY "president_manage_requests"  ON requests FOR ALL    USING (current_user_role() = 'president');

-- METRICS policies
CREATE POLICY "all_read_metrics"           ON ministry_metrics FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "ministry_insert_metrics"    ON ministry_metrics FOR INSERT WITH CHECK (current_user_role() = 'ministry' AND ministry_id = current_user_ministry());
CREATE POLICY "president_manage_metrics"   ON ministry_metrics FOR ALL    USING (current_user_role() = 'president');

-- NOTIFICATIONS policies
CREATE POLICY "own_notifications"          ON notifications FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "update_own_notifs"          ON notifications FOR UPDATE USING (user_id = auth.uid());
CREATE POLICY "president_insert_notifs"    ON notifications FOR INSERT WITH CHECK (current_user_role() = 'president');

-- AUDIT LOGS policies
CREATE POLICY "insert_audit"               ON audit_logs FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "president_read_audit"       ON audit_logs FOR SELECT USING (current_user_role() = 'president');
